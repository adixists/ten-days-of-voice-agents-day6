...
1. Introduce itself as the bank's fraud department.
2. Verify the customer in a basic, safe way.
3. Read out the suspicious transaction.
4. Ask if it was actually made by the customer.
5. Mark the case as safe or fraudulent.
...
#### Create a sample fraud cases in Database
...
#### Set up the fraud agent persona
...
#### Load the fraud case at call start
...
#### Implement a simple call flow

Design a clear, minimal sequence:

1. Greet the user and explain why the bank is calling.
2. Ask a basic verification question (using data), such as:
...
6. End the call with a short confirmation of what action was taken.

#### Update the fraud case in database
...
Run your fraud alert agent over a real phone call via LiveKit Telephony, with fraud cases loaded from database and results persisted back to database.
